...
def preprocess_frequency(X_train, X_test=None, fit_mode=True):
    ...
    X_train_processed['freq_log_norm'] = freq_scaler.transform(X_train_processed[['freq_log']])
    if X_test_processed is not None:
        X_test_processed['freq_log_norm'] = freq_scaler.transform(X_test_processed[['freq_log']])
    ...
    for data in [X_train_processed, X_test_processed]:
        ...
        data['freq_band_1'] = (data['freq'] < 1e9).astype(int)                          # < 1 GHz (UHF and below)
        data['freq_band_2'] = ((data['freq'] >= 1e9) & (data['freq'] < 6e9)).astype(int)  # 1-6 GHz (L/S bands)
        data['freq_band_3'] = ((data['freq'] >= 6e9) & (data['freq'] < 20e9)).astype(int) # 6-20 GHz (C/X/Ku bands)
        data['freq_band_4'] = ((data['freq'] >= 20e9) & (data['freq'] < 40e9)).astype(int) # 20-40 GHz (K/Ka bands)
        data['freq_band_5'] = (data['freq'] >= 40e9).astype(int)                        # > 40 GHz (Q/V/W bands)
    ...
    for data in [X_train_processed, X_test_processed]:
        ...
        data['freq_inv'] = 1.0 / (data['freq_log_norm'] + 1e-10)  # Add small epsilon to avoid division by zero
    ...
    for data in [X_train_processed, X_test_processed]:
        ...
        for l in ['DEV_L_0_9um', 'DEV_L_2_5um', 'DEV_L_5_0um']:
            if l in data.columns:
                data[f'freq_{l}_interaction'] = data['freq_log_norm'] * data[l]
    ...
    for data in [X_train_processed, X_test_processed]:
        ...
        for i in range(1, 6):
            ...
            if mask.sum() > 0:  # Only if there are samples in this band
                ...
                data.loc[mask, f'freq_pos_in_band_{i}'] = (data.loc[mask, 'freq'] - lower_bound) / (upper_bound - lower_bound)
    ...
    for data in [X_train_processed, X_test_processed]:
        ...
        if 'Zin_real_log' in data.columns and 'Zin_imag_log' in data.columns:
            for i in range(1, 6):
                data[f'band_{i}_Zin_real'] = data[f'freq_band_{i}'] * data['Zin_real_log']
                data[f'band_{i}_Zin_imag'] = data[f'freq_band_{i}'] * data['Zin_imag_log']
